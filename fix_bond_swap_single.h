...
FixStyle(bond/swap/single,FixBondSwapSingle)
...
class FixBondSwapSingle : public Fix {
 public:
  FixBondSwapSingle(class LAMMPS *, int, char **);
  ~FixBondSwapSingle();
  int setmask();
  void init();
...
E: Cannot use fix bond/swap/single with non-molecular systems
...
not be invoked by fix bond/swap/single.
...
E: Fix bond/swap/single cannot use dihedral or improper styles

These styles cannot be defined when using this fix.

E: Fix bond/swap/single requires special_bonds = 0,1,1
