/* ----------------------------------------------------------------------
   ...
   This fix is modifed based on fix_bond_swap.cpp
   This fix is used for bond swapping in the single chain
------------------------------------------------------------------------- */
...
// modification
FixBondSwapSingle::FixBondSwapSingle(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  ...
  temperature(NULL), random(NULL)
{
  ...
  if (narg != 7) error->all(FLERR,"Illegal fix bond/swap/single command");

  nevery = force->inumeric(FLERR,arg[3]);
  if (nevery <= 0) error->all(FLERR,"Illegal fix bond/swap/single command");
...
// modification
FixBondSwapSingle::~FixBondSwapSingle()
{
  delete random;
...
// modification
void FixBondSwapSingle::init()
{
  // require an atom style with molecule IDs

  if (atom->molecule == NULL)
    error->all(FLERR,
               "Must use atom style with molecule IDs with fix bond/swap/single");

  int icompute = modify->find_compute(id_temp);
  if (icompute < 0)
    error->all(FLERR,"Temperature ID for fix bond/swap/single does not exist");
  ...
  if (force->pair == NULL || force->bond == NULL)
    error->all(FLERR,"Fix bond/swap/single requires pair and bond styles");

  if (force->pair->single_enable == 0)
    error->all(FLERR,"Pair style does not support fix bond/swap/single");

  if (force->angle == NULL && atom->nangles > 0 && comm->me == 0)
    error->warning(FLERR,"Fix bond/swap/single will ignore defined angles");

  if (force->dihedral || force->improper)
    error->all(FLERR,"Fix bond/swap/single cannot use dihedral or improper styles");

  if (force->special_lj[1] != 0.0 || force->special_lj[2] != 1.0 ||
      force->special_lj[3] != 1.0)
    error->all(FLERR,"Fix bond/swap/single requires special_bonds = 0,1,1");
...
void FixBondSwapSingle::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixBondSwapSingle::post_integrate()
{
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      for (ibond = 0; ibond < num_bond[i]; ibond++) {
        ...
        for (jbond = 0; jbond < num_bond[j]; jbond++) {
          ...
          // use the iterative DFS algorithm to find the path between i and j
          // check whether inext, jnext are in the between
          // if they are not or both of them are
          // then the swap attempt is skipped
          // only when one of inext and jnext in the between
          // attemp the swap
          ...
          //   E_nb(i,jnext) + E_nb(j,inext) + E_nb(j,jnext) + E_nb(i,inext) +
          //   E_bond(i,j) + E_bond(jnext,inext) +
          //   E_angle(iprev,i,j) + E_angle(i,j,jprev) +
          //   E_angle(ilast,inext,jnext) + E_angle(inext,jnext,jlast)
          ...
          delta += bond_eng(ibondtype,i,j) + bond_eng(jbondtype,jnext,inext) -
            bond_eng(ibondtype,i,inext) - bond_eng(jbondtype,j,jnext);
          if (angleflag)
            delta += angle_eng(iangletype,iprev,i,j) +
              angle_eng(jangletype,i,j,jprev) +
              angle_eng(jnextangletype,inext,jnext,jlast) +
              angle_eng(inextangletype,ilast,inext,jnext) -
  ...
  // on atom inext:
  //   angle iprev-i-inext changes to jlast-jnext-inext
  //   angle i-inext-ilast changes to jnext-inext-ilast
  // on atom jnext:
  //   angle jprev-j-jnext changes to ilast-inext-jnext
  //   angle j-jnext-jlast changes to inext-jnext-jlast
  ...
  for (iangle = 0; iangle < num_angle[i]; iangle++) {
    ...
    if (i1 == iprevtag && i2 == itag && i3 == inexttag)
      angle_atom3[i][iangle] = jtag;
    else if (i1 == inexttag && i2 == itag && i3 == iprevtag)
      angle_atom1[i][iangle] = jtag;
    else if (i1 == itag && i2 == inexttag && i3 == ilasttag) {
      angle_atom2[i][iangle] = jtag;
      angle_atom3[i][iangle] = jprevtag;
    } else if (i1 == ilasttag && i2 == inexttag && i3 == itag) {
      angle_atom1[i][iangle] = jprevtag;
      angle_atom2[i][iangle] = jtag;
    }
  }
  ...
  for (jangle = 0; jangle < num_angle[j]; jangle++) {
    ...
    if (j1 == jprevtag && j2 == jtag && j3 == jnexttag)
      angle_atom3[j][jangle] = itag;
    else if (j1 == jnexttag && j2 == jtag && j3 == jprevtag)
      angle_atom1[j][jangle] = itag;
    else if (j1 == jtag && j2 == jnexttag && j3 == jlasttag) {
      angle_atom2[j][jangle] = itag;
      angle_atom3[j][jangle] = iprevtag;
    } else if (j1 == jlasttag && j2 == jnexttag && j3 == jtag) {
      angle_atom1[j][jangle] = iprevtag;
      angle_atom2[j][jangle] = itag;
    }
  }
  ...
  for (iangle = 0; iangle < num_angle[inext]; iangle++) {
    ...
    if (i1 == iprevtag && i2 == itag && i3 == inexttag) {
      angle_atom1[inext][iangle] = jlasttag;
      angle_atom2[inext][iangle] = jnexttag;
    } else if (i1 == inexttag && i2 == itag && i3 == iprevtag) {
      angle_atom2[inext][iangle] = jnexttag;
      angle_atom3[inext][iangle] = jlasttag;
    } else if (i1 == itag && i2 == inexttag && i3 == ilasttag)
      angle_atom1[inext][iangle] = jnexttag;
    else if (i1 == ilasttag && i2 == inexttag && i3 == itag)
      angle_atom3[inext][iangle] = jnexttag;
  }
  ...
  for (jangle = 0; jangle < num_angle[jnext]; jangle++) {
    ...
    if (j1 == jprevtag && j2 == jtag && j3 == jnexttag) {
      angle_atom1[jnext][jangle] = ilasttag;
      angle_atom2[jnext][jangle] = inexttag;
    } else if (j1 == jnexttag && j2 == jtag && j3 == jprevtag) {
      angle_atom2[jnext][jangle] = inexttag;
      angle_atom3[jnext][jangle] = ilasttag;
    } else if (j1 == jtag && j2 == jnexttag && j3 == jlasttag)
      angle_atom1[jnext][jangle] = inexttag;
    else if (j1 == jlasttag && j2 == jnexttag && j3 == jtag)
      angle_atom3[jnext][jangle] = inexttag;
  }
  ...
  // on atom ilast: angle i-inext-ilast changes to jnext-inext-ilast
  // on atom jlast: angle j-jnext-jlast changes to inext-jnext-jlast
  ...
  for (iangle = 0; iangle < num_angle[iprev]; iangle++) {
    ...
    if (i1 == iprevtag && i2 == itag && i3 == inexttag)
      angle_atom3[iprev][iangle] = jtag;
    else if (i1 == inexttag && i2 == itag && i3 == iprevtag)
      angle_atom1[iprev][iangle] = jtag;
  }
  ...
  for (jangle = 0; jangle < num_angle[jprev]; jangle++) {
    ...
    if (j1 == jprevtag && j2 == jtag && j3 == jnexttag)
      angle_atom3[jprev][jangle] = itag;
    else if (j1 == jnexttag && j2 == jtag && j3 == jprevtag)
      angle_atom1[jprev][jangle] = itag;
  }
  ...
  for (iangle = 0; iangle < num_angle[ilast]; iangle++) {
    ...
    if (i1 == itag && i2 == inexttag && i3 == ilasttag)
      angle_atom1[ilast][iangle] = jnexttag;
    else if (i1 == ilasttag && i2 == inexttag && i3 == itag)
      angle_atom3[ilast][iangle] = jnexttag;
  }
  ...
  for (jangle = 0; jangle < num_angle[jlast]; jangle++) {
    ...
    if (j1 == jtag && j2 == jnexttag && j3 == jlasttag)
      angle_atom1[jlast][jangle] = inexttag;
    else if (j1 == jlasttag && j2 == jnexttag && j3 == jtag)
      angle_atom3[jlast][jangle] = inexttag;
  }
}

/* ---------------------------------------------------------------------- */

int FixBondSwapSingle::modify_param(int narg, char **arg)
{
...
double FixBondSwapSingle::dist_rsq(int i, int j)
{
...
double FixBondSwapSingle::pair_eng(int i, int j)
{
  double tmp;
...
double FixBondSwapSingle::bond_eng(int btype, int i, int j)
{
  double tmp;
  double rsq = dist_rsq(i,j);
...
double FixBondSwapSingle::angle_eng(int atype, int i, int j, int k)
{
...
double FixBondSwapSingle::compute_vector(int n)
{
  double one,all;
  if (n == 0) one = naccept;
  else one = foursome;
...
double FixBondSwapSingle::memory_usage()
{
